...
addpath(genpath('C:\D\Other\Notes\Lightspeed Matlab toolbox\lightspeed'));

% INITIALISATION AND PARAMETERS:
% ==============================

no_of_runs = 30;            % number of experiments to generate statistical
                            % averages
doPlot = 0;                 % 1 plot online. 0 = only plot at the end.
sigma =  1e-1;              % Variance of the Gaussian measurement noise.
...
clutter_min=40; clutter_max=50; % for generating clutters
...
for j=1:no_of_runs
    ...
    for t=2:T
        ...
        measureNoise(t) = sqrt(sigma)*randn(1,1);         % no clutter
...
figure(6),plot(1:T,pai_ave(:,1)',1:T,pai_ave(:,2)');grid on;legend('Gaussian model','Student model (v=3)');xlabel('time');ylabel('posterior probability');
